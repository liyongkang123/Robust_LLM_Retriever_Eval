...
Support dense retrieval and BM25 retrieval methods
"""
...
#         break
#     print(f'Compressed retrieval results from top-{k_old} to top-{k_new}.')
#     return sub_results
...
def config_parse():
    ...
    parser.add_argument("--model_name", type=str, default="reasonir", help="Path to the evaluation model. bge_reasoner reasonir gte qwen3 linq contriever bge_m3  diver other models can be run normally")
...
def BM25_retrieval(config):
    ...
    # The code below is from the code of resonir
    ...
    model = LuceneBM25Model(dictionary=dictionary, k1=0.9, b=0.4) # Here it follows the BM25 “Flat” baseline
    ...
    # Execute retrieval
    results = {}
    for query_id, query_text in tqdm(queries.items(), desc="BM25检索"):
        ...
        # Get top-k results
        doc_scores = dict(zip(doc_ids, similarities))
        top_docs = sorted(doc_scores.items(), key=lambda x: x[1], reverse=True)[:1000]
        results[str(query_id)] = dict(top_docs)
    ...
    for split_name in splits:
        split_qrels = qrels[split_name]
        split_results = {qid: docs for qid, docs in results.items() if qid in split_qrels} # 只保留 split_qrels 中的结果
        _evaluate_split(split_name, split_results, split_qrels, config, experiment_setup['scores_save_root'], experiment_setup['wandb_run'])

    # Clean up
    if experiment_setup['wandb_run']:
        wandb.finish()
    logger.info("BM25 evaluation completed")
...
if __name__ == '__main__':
    config = config_parse()
    if config.model_name == "bm25":
        BM25_retrieval(config)
    else:
